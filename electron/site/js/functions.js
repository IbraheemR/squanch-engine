...
function loadUserStory() {
  ...
  for (storynum in stories) {
    writeLine(" s& " + String(Number(storynum) + 1) + ". &s  " +  stories[storynum].name);
  }

  setPrompt("Chose an adventure!")
  revealTextField();
  setQuestion("Which number?", (answer) => {

  let number = Number(answer);
  if (isNaN(number)) {
    setQuestion("Not a valid number, try again!");
  }
  else if (number > stories.length) {
    setQuestion("Number to big, try again!");
  }
  else {
    concealInput();
...
function fadeInText(func) {
  if (func) {
    $("#body.text").fadeIn(400, func);
  }
  else {
    $("#body.text").fadeIn();
  }
...
function fadeOutText(func) {
  if (func) {
    $("#body.text").fadeOut(400, func);
  }
  else {
    $("#body.text").fadeOut();
  }
